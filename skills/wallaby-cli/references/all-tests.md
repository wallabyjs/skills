...
- `- status:` is always present in `all-tests.md` and is one of `passed`, `failed`, `skipped`, or `todo`.
...
- `all-tests.md` includes passed, failed, skipped, and todo tests.
...
- Use `failing-tests.md` or the main report when failure diagnostics are needed instead of inventory.
